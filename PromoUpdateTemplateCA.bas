Attribute VB_Name = "PromoUpdateTemplateCA"
Option Explicit

'=====================================================================================
' PromoUpdateTemplateCA.bas
'
' Builds the SAP IBP "Promo Update Template" for CANADA.
'
' This is a standalone module for the Canadian workbook.  It shares no state, no sheets
' and no procedures with the US build - import it into the Canadian file and run it
' there.  Do not import both modules into the same workbook.
'
' Inputs (read only, never modified), in the Canadian workbook:
'
'     Promo Grid            product x week promo net prices - the SOURCE OF TRUTH for
'                           both MSRP and the week calendar
'     CY Template           last year's submitted template (channels, policy price, %)
'     NY Template           header row only; supplies the non-week column layout
...
'
' Entry point: GeneratePromoUpdateTemplateCA.
'
'-------------------------------------------------------------------------------------
' HOW THIS DIFFERS FROM THE US BUILD
'
'  A. The week calendar comes from the Promo Grid, not from the NY Template.  Each grid
'     week date is converted to a WB caption - 4/1/2026 becomes "WB APR-01 2026" - and
'     that caption becomes an output column.  In the US build a grid week with no
'     matching NY column was skipped; here it is created.  NY Template WB columns with
'     no matching grid week are left out.  Both directions are summarised in the Run Log.
'
'  B. Output column order is the NY Template's non-week columns in their original order,
'     followed by the grid-derived week columns in date order.
'
'  C. MSRP always comes from the Promo Grid.  The column is required.  A CY Template MSRP
'     that disagrees is logged, and the grid value wins.
'
'  D. Month abbreviations in WB captions are generated from an internal table rather than
'     Format(), so a French-Canadian Excel locale cannot produce "AVR" instead of "APR".
'
'  E. Config Country defaults to CA, and a run against a workbook whose Config says
'     otherwise is logged as a warning.
'
'  Canadian events (Boxing Day and the like) and Canadian funding accounts need no code:
'  events are read from the Full Funding Events sheet and accounts from Config.
'
'-------------------------------------------------------------------------------------
' ASSUMPTIONS
...
'  4. A week column on the Promo Grid is any header that parses to a date.  Grid week
'     dates need not fall on any particular weekday.
'  5. A week is seven days: a block spans its first week's date through its last week's
'     date + 6.  Consecutive grid weeks that are not 7 days apart are logged once.
...
'     through a depth, does.  Funding windows are intersected with the weeks actually
'     promoted at that depth, so an event falling in a gap cannot split the row.
...
'     the Model Desc (case-insensitive substring).
' 10. Policy Price comes from the CY row for that product x channel; if it is missing
'     or zero the Promo Grid MSRP is substituted and logged.
...
' 12. Currency is rounded half-up to 2 decimals, in the workbook's own currency - no
'     FX conversion is performed anywhere.  Special promo dates are written as text in
'     the Config date format so the blank placeholder can share the column.
' 13. Output is values only, never formulas.  NY columns that are neither recognised
'     nor WB week columns are carried through and left blank.
'=====================================================================================
...
'--- NY template classification and the output column layout --------------------------
Private mNYWkByDate As Object
Private mNYIsWB() As Boolean
Private mNYToOut() As Long
Private mOutCapt() As String

'--- Promo Grid week calendar (chronological) -----------------------------------------
Private mWkDate() As Double
Private mWkGridCol() As Long
Private mWkOutCol() As Long
...
' ENTRY POINT
'=====================================================================================
Public Sub GeneratePromoUpdateTemplateCA()
    Dim wb As Workbook
    ...
    mStage = "building the output column layout from the Promo Grid calendar"
    BuildLayout
...
Private Sub ReadConfig(ByVal wb As Workbook)
    ...
    If Len(mCfgCountry) = 0 Then mCfgCountry = "CA"
    If UCase$(Left$(mCfgCountry, 2)) <> "CA" Then
        LogIssue "Country is not Canada", "", "", "", "This is the Canada build but Config 'Country' is '" & mCfgCountry & "'. Check that the active workbook is the Canadian one; '" & mCfgCountry & "' was written into every output row."
    End If
...
Private Sub ReadGrid(ByVal wb As Workbook)
    ...
    mGCMSRP = ReqCol(mGridCol, "MSRP", "Promo Grid")
    If mGCModel = 0 Then
    ...
    ReDim mWkOutCol(1 To mGridCols)
    ...
    '--- a WB week is seven days; flag a calendar that is not
    For i = 2 To mWkCount
        If Abs((mWkDate(i) - mWkDate(i - 1)) - 7) > 0.0001 Then
            LogOnce "WKGAP", "Week calendar is not 7-day", "", "", FmtD(mWkDate(i - 1)) & " -> " & FmtD(mWkDate(i)), "Consecutive Promo Grid week columns are " & CLng(mWkDate(i) - mWkDate(i - 1)) & " days apart, not 7. Promo blocks still treat every week as seven days, so check the grid calendar."
        End If
    Next i
End Sub
...
Private Sub ReadNY(ByVal wb As Workbook)
    ...
    '--- classify, for the layout and for the calendar comparison in the Run Log.
    '--- The NY WB columns do NOT define the calendar here; the Promo Grid does.
    ReDim mNYIsWB(1 To mNYCols)
    wkCols = 0
    For c = 1 To mNYCols
        mNYIsWB(c) = False
        cap = CStr2(mNY(1, c))
    ...
    If wkCols = 0 Then
        LogIssue "NY Template has no WB columns", "", "", "", "The NY Template header carries no 'WB' week columns, so the whole week calendar was generated from the Promo Grid dates."
    End If
...
Private Sub BuildLayout()
    ...
    ReDim mNYToOut(1 To mNYCols)
    ReDim mOutCapt(1 To mNYCols + mWkCount)
    ReDim mWkOutCol(1 To mWkCount)

    '--- the NY Template's non-week columns, in their original order
    n = 0
    seenWB = False
    moved = 0
    For c = 1 To mNYCols
        ...
        Else
            n = n + 1
            mOutCapt(n) = CStr2(mNY(1, c))
            mNYToOut(c) = n
            If seenWB Then moved = moved + 1
        End If
    Next c

    '--- then one week column per Promo Grid week, in date order
    matched = 0
    added = 0
    sample = ""
    For i = 1 To mWkCount
        n = n + 1
        mOutCapt(n) = WBCaption(mWkDate(i))
        mWkCap(i) = mOutCapt(n)
        mWkOutCol(i) = n
    ...
    dropped = mNYWkByDate.Count - matched
    If dropped < 0 Then dropped = 0

    LogIssue "Week calendar taken from Promo Grid", "", "", "", mWkCount & " week column(s) were generated from the Promo Grid dates: " & matched & " already existed on the NY Template, " & added & " were added, and " & dropped & " NY Template week(s) had no Promo Grid column and were left out."
    If added > 0 Then
        extra = ""
        If added > 8 Then extra = " (and " & (added - 8) & " more)"
        LogIssue "Week columns added", "", "", "", "Weeks generated from the Promo Grid that the NY Template did not carry: " & sample & extra
    End If
    If moved > 0 Then
        LogIssue "NY column order changed", "", "", "", moved & " non-week column(s) sat after the NY Template's WB block. They keep their original relative order but now appear before the week columns."
    End If
End Sub

'--- 4/1/2026 -> "WB APR-01 2026".  Built from an internal month table rather than
'--- Format(), so a French-Canadian locale cannot yield "AVR" instead of "APR".
Private Function WBCaption(ByVal d As Double) As String
    Dim dt As Date
    dt = CDate(d)
    WBCaption = "WB " & MonthAbbr(Month(dt)) & "-" & Right$("0" & CStr(Day(dt)), 2) & " " & CStr(Year(dt))
End Function

Private Function MonthAbbr(ByVal m As Long) As String
    Select Case m
...
Private Sub ReadCY(ByVal wb As Workbook)
    ...
    End If
    If mCCMSRP = 0 Then
        LogIssue "Optional column missing", "", "", "", "The CY Template has no 'MSRP' column. The Promo Grid is the source of truth for MSRP in Canada, so nothing was lost."
    End If
    If mCCPolID = 0 Then
...
Private Sub EmitDepth(ByVal prod As String, ByVal model As String, ByVal gridMSRP As Double, ByVal price As Double)
    ...
    For ci = LBound(custs) To UBound(custs)
        ...
        If Len(cust) > 0 Then
            ...
            '--- the Promo Grid is the source of truth for MSRP in Canada
            useMSRP = gridMSRP
            If mCCMSRP > 0 Then
                If CDbl(mChanMSRP(key)) > 0 Then
                    If Abs(CDbl(mChanMSRP(key)) - gridMSRP) > 0.005 Then
                        LogOnce "MSRPDIFF" & key, "CY MSRP <> grid MSRP", prod, cust, "", "The CY Template MSRP is " & Format$(CDbl(mChanMSRP(key)), "0.00") & " but the Promo Grid MSRP is " & Format$(gridMSRP, "0.00") & ". The Promo Grid wins in Canada, so " & Format$(gridMSRP, "0.00") & " was written."
                    End If
                End If
            End If
            ...
            For s = 1 To mSegCount
                ...
                For w = 1 To mBlkCount
                    mOut(mOutRow, mWkOutCol(mBlkWk(w))) = RoundHalfUp(price)
                Next w
            Next s
        End If
    Next ci
...
Private Sub SetOut(ByVal canon As String, ByVal v As Variant)
    ...
    If c > mNYCols Then Exit Sub
    oc = mNYToOut(c)
    If oc < 1 Then Exit Sub
    If oc > mOutCols Then Exit Sub
    mOut(mOutRow, oc) = v
End Sub
...
Private Sub WriteOutput(ByVal wb As Workbook)
    ...
    SetColFormat ws, "MSRP", "0.00"
    SetColFormat ws, "Policy Price", "0.00"
    SetColFormat ws, "Epson Cont.", "0.00"
    SetColFormat ws, "Accn Cont.", "0.00"
    SetColFormat ws, "Net", "0.00"
    SetColFormat ws, "Special Promo Start Date", "@"
    SetColFormat ws, "Special Promo End Date", "@"
    SetColFormat ws, "Financial Year", "@"
    SetColFormat ws, "Start Week", "@"
    For i = 1 To mWkCount
        ws.Columns(mWkOutCol(i)).NumberFormat = "0.00"
    Next i

    '--- header row: the NY Template's non-week captions, then the grid's week captions
    For c = 1 To mOutCols
        ws.Cells(1, c).Value = mOutCapt(c)
    Next c
...
Private Sub SetColFormat(ByVal ws As Worksheet, ByVal canon As String, ByVal fmt As String)
    Dim c As Long
    Dim oc As Long
    ...
    If c > mNYCols Then Exit Sub
    oc = mNYToOut(c)
    If oc > 0 Then ws.Columns(oc).NumberFormat = fmt
End Sub
...
Private Sub WriteLog(ByVal wb As Workbook)
    ...
    ws.Cells(r + 8, 1).Value = "Grid weeks in calendar"
