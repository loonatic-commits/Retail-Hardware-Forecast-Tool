...
' =============================================================
' modMktgVsFF - Marketing Demand Fcst vs Field Forecast
'
...
Private Const KF_MKTG As String = "Marketing Forecast Qty"

' Variance = Mktg - FF. True divides by FF; False divides by Mktg.
...
Public Sub Build_Mktg_vs_FF()
    ...
    rep.Cells(1, 1).Value = "Marketing Demand Forecast vs Field Forecast - " & winLabel
    ...
    For Each b In blocks
        ...
        Dim mTot As Double, fTot As Double
        mTot = SumOver(ws, mktgRow, months, iFrom, iTo)
        fTot = SumOver(ws, ffRow, months, iFrom, iTo)

        Dim vUnits As Double, vPct As Double, havePct As Boolean
        vUnits = mTot - fTot
        havePct = Pct(mTot, fTot, vPct)

        rep.Cells(r, 2).Value = mTot
...
NextBlock:
    ...
    r = MonthlyTable(rep, ws, blocks, months, iFrom, iTo, r, _
                     ...
                     IIf(PCT_OF_FF, "of Field Forecast", "of Marketing"), True)
...
Private Function WhyText(ws As Worksheet, ByVal block As Object, ByVal months As Variant, _
    ...
    If havePct Then
        If Abs(vPct) >= MATERIAL Then
            s = AppendSemi(s, "marketing sits " & Format(Abs(vPct), "0.0%") & " " & _
                              IIf(vPct > 0, "above", "below") & " field over the window, " & _
                              "past the " & Format(MATERIAL, "0%") & " bar - worth a look")
        Else
            s = AppendSemi(s, "marketing and field within " & Format(MATERIAL, "0%") & _
                              " over the window")
        End If
    End If
...
Private Function Pct(ByVal mktg As Double, ByVal ff As Double, ByRef outPct As Double) As Boolean
    Dim denom As Double
    If PCT_OF_FF Then denom = ff Else denom = mktg
