...
' Regression guard. VBA's IsDate() accepts "27-Mar" and CDate()
' reads it as 27 March of the CURRENT year, so a header meaning
' March 2027 silently became March 2026. Every yy-mmm header
' past the year boundary was affected.
Private Sub Check_MonthParsing()
    Debug.Print "-- Month header parsing"

    AssertMonth "26-Aug", 2026, 8
    AssertMonth "27-Mar", 2027, 3        ' the one that broke
    AssertMonth "27-Jan", 2027, 1
    AssertMonth "26-Apr", 2026, 4
    AssertMonth "Mar-27", 2027, 3
    AssertMonth "MAY 2026", 2026, 5
    AssertMonth "JUN 2026", 2026, 6

    ' A real Date cell must still pass straight through.
    Dim d As Date, got As Date
    d = DateSerial(2027, 2, 1)
    If TryParseMonth(d, got) Then
        AssertLong "real Date cell year", Year(got), 2027
        AssertLong "real Date cell month", Month(got), 2
    Else
        Debug.Print "  FAIL  real Date cell did not parse"
        mFails = mFails + 1
    End If
End Sub

Private Sub AssertMonth(ByVal header As String, ByVal wantYear As Long, ByVal wantMonth As Long)
    Dim d As Date
    If Not TryParseMonth(header, d) Then
        Debug.Print "  FAIL  '" & header & "' did not parse at all"
        mFails = mFails + 1
        Exit Sub
    End If
    If Year(d) = wantYear And Month(d) = wantMonth Then
        Debug.Print "  PASS  '" & header & "' -> " & Format(d, "mmm yyyy")
    Else
        Debug.Print "  FAIL  '" & header & "' -> " & Format(d, "mmm yyyy") & _
                    "  expected " & Format(DateSerial(wantYear, wantMonth, 1), "mmm yyyy")
        mFails = mFails + 1
    End If
End Sub
