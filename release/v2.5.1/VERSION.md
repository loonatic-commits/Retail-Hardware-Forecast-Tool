# VBA Demand Forecasting Suite — v2.5.1
...
| `Build_Mktg_vs_FF` | Marketing vs Field variance sheet, Aug–Mar. Standalone and read-only. |
| `Show_Month_Headers` | Diagnostic — dumps how row 1 of Consensus is being parsed. |
...
| `modMktgVsFF.bas` | Standalone — Marketing vs Field variance summary |
...
Only `modMktgVsFF.bas` changed — it was comparing the wrong row.

The sheet is now **`Opportunity Qty Final` vs `Field Forecast Qty Final`**.
It previously compared `Marketing Forecast Qty`, which is a different row.

```
Variance = Opportunity Qty Final − Field Forecast Qty Final
Percent  = that variance ÷ Field Forecast Qty Final
```

Both row names are `KF_LEFT` and `KF_FIELD_FCST_FINAL` at the top of the
module. Headers, table titles and the rationale wording all follow the change,
and the sheet states which two rows it compared in its subtitle.

---

## What changed in v2.5.0

`modVarianceSummary.bas` is renamed **`modMktgVsFF.bas`**. Delete the old
module — if both are loaded you may run the old one by mistake.

| Old | New |
|---|---|
| module `modVarianceSummary` | module `modMktgVsFF` |
| `Build_Variance_Summary` | `Build_Mktg_vs_FF` |
| — | `Show_Month_Headers` (new diagnostic) |

### It now parses month headers itself

The v2.3.1 fix to `TryParseMonth` lives in `modUtilities`. If that module was
not actually replaced — VBA keeps the existing module when you import over it
without removing it first — the old buggy parser is still what runs, and the
Aug–Mar window still cannot be found.

`modMktgVsFF` no longer calls `modUtilities` for month parsing. It carries its
own year-aware parser, so a stale `modUtilities` cannot affect it. The parser
also handles `/` separators, `2026-08` / `08-2026` numeric forms, bare date
serials, non-breaking spaces and tabs.

### The error message now shows its work

If the window is not found, the dialog names which end is missing and lists
every month it *did* parse off the sheet. `Show_Month_Headers` goes further —
per column it prints the raw cell value, its VBA type, and what the parser
made of it, to the Immediate window and a summary dialog.

That turns "could not find the window" from a dead end into a one-run
diagnosis: if `27-Mar` shows as `Mar 2026`, a stale parser is running; if it
shows `NOT PARSED`, the header is not in a form the parser recognises; if the
month simply is not listed, it is not on the sheet.
