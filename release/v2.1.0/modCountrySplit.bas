...
' 'Country Split' sheet.
'
'   - Canada values are read AS-IS and never modified.
...
' number, so the subtraction uses an upratedvalue instead:
'
...
'
' Note: the uplift changes what is SUBTRACTED, not the Canada row
' itself, so in an uplifted month CANADA + USA will not add back
' to the NA total. Set WRITE_UPLIFT_TO_CANADA to True if you would
' rather the Canada row be restated so the two halves reconcile.
'
...
Private Const WRITE_UPLIFT_TO_CANADA As Boolean = False
                        ...
                        If caSoFar > 0 Then
                            If caSoFar >= caVal * SOFAR_CLOSE_RATIO Then
                                ...
                                Debug.Print "CountrySplit: '" & modelName & "' current month uplift - " & _
                                            ...
                                            " -> subtracting " & Format(caEffective, "#,##0")
