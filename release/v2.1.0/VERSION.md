# VBA Demand Forecasting Suite — v2.1.0
...
| `WRITE_UPLIFT_TO_CANADA` | `False` | Leave the Canada row untouched |

**The uplift changes what is subtracted, not the Canada row itself.** In an
uplifted month, CANADA + USA will not add back to the NA total. Flip
`WRITE_UPLIFT_TO_CANADA` to `True` if you would rather the Canada row be
restated so the two halves reconcile.
